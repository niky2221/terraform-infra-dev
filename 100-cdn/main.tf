resource "aws_cloudfront_distribution" "expense94" {
  origin {
  ...
  viewer_certificate {
    acm_certificate_arn = data.aws_ssm_parameter.web_alb_certificate.arn
...
resource "aws_route53_record" "cloudfront" {
  zone_id  = var.zone_id
  name     = "${var.project}-cdn.${var.domain_name}"
  type     = "A"

  alias {
    name                   = aws_cloudfront_distribution.expense94.domain_name
    zone_id                = aws_cloudfront_distribution.expense94.hosted_zone_id
